...
// Currency Icons
// ============================================================
function CurrencyDisplay({ diamonds, coins, energy, scrolls }: {
  diamonds: number; coins: number; energy: number; scrolls: number;
}) {
  return (
...
// Top App Bar
function TopAppBar() {
  const { coins, diamonds, energy, scrolls } = useCollectionStore();
  ...
  return (
    <div className="sticky top-0 z-40 px-4 h-[72px] flex items-center justify-between" style={{ backgroundColor: colors.background }}>
      ...
      <CurrencyDisplay diamonds={diamonds} coins={coins} energy={energy} scrolls={scrolls} />
    </div>
  );
}
